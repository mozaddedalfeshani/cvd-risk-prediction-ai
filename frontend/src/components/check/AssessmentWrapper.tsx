...
export default function AssessmentWrapper() {
  ...
  const handlePrediction = async (patientData: PredictPayload) => {
    ...
    try {
      const response = await fetch("http://54.160.253.120:5001/api/predict", {
        method: "POST",
        headers: {
    ...
    <>
      ...
      <motion.div
        ...
        <motion.div
          ...
          <span className="font-bold">{t.features.dual_ai}</span>
          <span className="w-2 h-2 bg-purple-400 rounded-full mx-2" />
          <span className="font-bold">95.9% Accuracy!</span>
        </motion.div>
      </motion.div>
      ...
      <motion.div
        ...
        <p>
          Made with 💖 for a healthier world. (Remember: This is an AI tool,
          always see a doctor! 👨‍⚕️)
        </p>
      </motion.div>
    </>
  );
